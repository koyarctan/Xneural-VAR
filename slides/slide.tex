...
次の3点を、同じ生成データ上で分けて検証する。
\begin{enumerate}
  \item 閾値を置かず、厳密ゼロからGranger因果グラフを得られるか
  \item 真のグラフをどの程度回収できるか
  \item 状態依存係数の符号を解釈できるか
\end{enumerate}
...
\begin{tabular}{llll}
\toprule
データ & 系列長・次元 & 真の設定 & 推定設定 \\
\midrule
線形VAR(1) & $T=300,\ p=10$ & 非自己edge 20本 & $K=5$, 5 data seeds \\
Lorenz-96 & $T=500,\ p=20$ & $F=10,40$ & $K=5$, 5 init. seeds \\
\bottomrule
\end{tabular}
\end{table}

\begin{itemize}
  \item 比較手法：GVAR、NGC(cMLP)、linear VAR。自己edgeは主評価から除外。
  \item neural modelは全データ・full batch、150 epoch。校正seedと評価seedを分離。
  \item Xneural VARとcMLPは正則化パスを再学習し、各$\lambda$の厳密ゼロを評価。
\end{itemize}
\end{frame}
...
\begin{itemize}
  ...
  \item 本実験：NGC著者コードに合わせ、$T=500$, $\Delta t=0.05$, burn-in 1000,
        観測雑音$\mathcal{N}(0,0.1^2)$を使用。
  \item GVAR論文でも$F=10,40$を使用。本比較では全手法へ同一のNGC生成データを与える。
  \item 真の非自己edgeは$j\in\{i+1,i-1,i-2\}$の60本。
\end{itemize}
\end{frame}

\begin{frame}{0/1 Granger因果グラフの判定規則}
\scriptsize
...
手法 & 判定規則 \\
\midrule
Xneural VAR & $\|G_{:,i,j}\|_2>0$ならedgeあり。数値閾値なし。 \\
NGC(cMLP) & 入力層のsource group normが厳密に0でなければedgeあり。 \\
GVAR & 元系列と時間反転系列の係数強度が最も整合する分位点を選択（$Q=20$）。 \\
linear VAR & sourceの全5ラグを同時に除くpartial F-test。BH法でFDR $q=0.05$。 \\
\bottomrule
\end{tabular}
\end{table}

\begin{itemize}
  \item \textbf{top-$k$や真のedge数は使用しない。}
  \item Xneural VAR/cMLPのAUROC・AUPRC：$\lambda$を変えて再学習し、
        edgeが厳密非ゼロで残る最大$\lambda$を順位scoreとする。
  \item linear VARの順位scoreは$-\log_{10}(p)$、GVARは係数強度。固定0/1グラフは上表の規則。
  \item 固定$\lambda$は別の校正seedでF1最大の値を選び、5評価seedへ固定する。
\end{itemize}
\end{frame}

\begin{frame}{Xneural VAR：厳密ゼロの正則化パス}
\centering
\includegraphics[width=0.83\linewidth]{lorenz-96_f40_xneural_path.png}

\vspace{-0.4em}
\scriptsize
$F=40$では$\lambda_{\mathrm{ngc}}=0.16$で平均75 edge、F1=0.685。
$0.17$以上では非自己edgeがすべて0となる。Xneural VARの5パスではedgeの再出現は0件。
\end{frame}

\begin{frame}{Lorenz-96：構造推定結果}
\centering
\scriptsize
\begin{table}
\centering
...
\toprule
$F$ & Method & AUROC & AUPRC & F1 & edge数 \\
\midrule
10 & Xneural VAR & 0.681 & 0.254 & 0.415 & 118.2 \\
10 & \textbf{NGC(cMLP)} & \textbf{0.991} & \textbf{0.954} & \textbf{0.910} & 60.4 \\
10 & GVAR & 0.578 & 0.220 & 0.235 & 102.2 \\
10 & linear VAR ($q=.05$) & 0.866 & 0.690 & 0.506 & 27.0 \\
\midrule
40 & Xneural VAR & 0.840 & 0.514 & 0.685 & 75.0 \\
40 & \textbf{NGC(cMLP)} & \textbf{0.954} & \textbf{0.792} & \textbf{0.744} & 96.0 \\
40 & GVAR & 0.641 & 0.238 & 0.284 & 235.6 \\
40 & linear VAR ($q=.05$) & 0.892 & 0.772 & 0.494 & 21.0 \\
\bottomrule
\end{tabular}
\end{table}

\begin{itemize}
  \item neural modelは5初期値の平均。95\% CIを含む全結果はnotebookに出力。
  \item Xneural VARはGVARを全指標で上回るが、NGC(cMLP)には及ばない。
  \item $F=40$ではlinear VARよりF1が高く、厳密ゼロの固定グラフで非線形構造を回収した。
\end{itemize}
\end{frame}

\begin{frame}{Lorenz-96 $(F=10)$：lag-wise causal gate}
  \centering
  \includegraphics[width=0.91\linewidth]{lorenz96_f10_gate_by_lag_benchmark.png}

  \vspace{-0.3em}
  \scriptsize 校正seedで選択した$\lambda_{\mathrm{ngc}}=0.16$、評価seed 0のgate。
\end{frame}

\begin{frame}{Lorenz-96 $(F=40)$：lag-wise causal gate}
  \centering
  \includegraphics[width=0.91\linewidth]{lorenz96_f40_gate_by_lag_benchmark.png}

  \vspace{-0.3em}
  \scriptsize
  図は自己edgeを含む学習gate全体。評価表のみ自己edgeを除外している。
\end{frame}

\begin{frame}{線形VAR(1)：設計と比較結果}
\centering
\scriptsize

$p=10,T=300$。正負の強いedge($|a|=0.34$)と弱いedge($|a|=0.20$)を各targetに置き、
相関したinnovation($\rho=0.15$)を加える。真の次数1に対して$K=5$で推定する。

\begin{table}
\centering
\begin{tabular}{lrrrrr}
\toprule
Method & AUROC & AUPRC & F1 & edge数 & sign acc. \\
\midrule
Xneural VAR ($\lambda=.12$) & 0.805 & 0.565 & 0.629 & 27.8 & 0.750 \\
\textbf{NGC(cMLP)} ($\lambda=.40$) & \textbf{0.976} & \textbf{0.924} & \textbf{0.866} & 19.8 & -- \\
GVAR & 0.570 & 0.286 & 0.329 & 52.4 & 0.930 \\
linear VAR (F-test, BH) & 0.951 & 0.896 & 0.752 & 14.0 & 1.000 \\
\bottomrule
\end{tabular}
\end{table}

\vspace{-0.2em}
sign acc.は真の非ゼロ係数について、有効係数の時点中央値と真の符号が一致した割合。
cMLPは入力層重みに符号付き有効係数の意味がないため対象外。
\end{frame}

\begin{frame}{線形VAR(1)：lag-wise causal gate}
  \centering
  \includegraphics[width=0.92\linewidth]{linear_var_gate_by_lag_benchmark.png}
\end{frame}

\begin{frame}{線形VAR(1)：有効係数の符号確認}
  \centering
  \includegraphics[width=0.95\linewidth]{linear_var_signed_boxplots_benchmark.png}

  \vspace{-0.3em}
  \scriptsize
  横軸はラグ、縦軸はcausal gateを通した時点別有効係数。
  0に潰れた真のedgeも含め、成功例だけを選んでいない。
\end{frame}
...
\begin{frame}{今後の課題}
  ...
  \begin{itemize}
    ...
    \item NGC(cMLP)との構造推定精度の差を縮める最適化・正則化設計。
    \item 実データへの適用可能性
  \end{itemize}
\end{frame}
